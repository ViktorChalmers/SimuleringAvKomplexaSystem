...
def P9(L,N,v,dt,noice,steps,r,h):
    ...
    for i in trange(steps):
        if i == 0 or i == 10 or i == 100 or i == 1000:
            ...
            plt.savefig(f"P9config;iter={i};r={r};noice={noice};N={N};h={h}".replace(".",","))
        fi[i] = getGlobalAlignment(velocity, v)
        cn[i] = globalClusteringCoefficient(position, L, r)

        flockIndex2 = np.copy(flockIndex)
        velocity2 = np.copy(velocity)
        position2 = np.copy(position)

        for j in range(-h):
            flockIndex2 = getFlockIndex(position2, L, r)
            theta = updateTheta(theta, flockIndex2, noice, dt)
            velocity2 = getVelocity(theta, v)
            position2 = updatePosition(position2, velocity2, dt, L)
        flockIndex = getFlockIndex(position, L, r)
    ...
    plt.savefig(f"P9fc;iter={steps};r={r};noice={noice};N={N};h={h}".replace(".",","))
